...
To illustrate: the current time in your timezone is <span className="font-mono"><LightningText part={LightningPart.Bolt} />~<LightningText
...
Because Lightning Time is just a variation on hexadecimal time, we can represent the current time as a color,
since any 6-digit hexadecimal string is also an HTML color. We don't want to represent the time as one color, though—if we did,
the color would often look bad. Instead, **Lightning Time represents time as three colors.**

A typical hexadecimal color looks something like <span className="font-bold color-demo">#5c10e8</span>. Many people see hexadecimal colors as some kind of magic color-producing code—but,
...
![](/posts/lightning-time/colors-unfilled.jpg)
...
This leaves the other two RGB values of each color open. So what are those filled in with? Whatever you want!

![](/posts/lightning-time/colors-filled.jpg)
...
You don't have to choose your own colors in order to use Lightning Time, though—we provide a set of default values that we expect
most people will use.

## Play with Lightning Time 🪁⚡️

That said, if you want to play around with Lightning Time colors & find a good color combination, check out
this tool 👇

\[embed color player here\]
...
This post is using that package! 😎 [Check it out on npm.](https://www.npmjs.com/package/@purduehackers/time)
...
![Image test](/posts/lightning-time/hack-night-brainstorming.jpeg)
...
real time in your day to build & ship technical projects. Coming together at Hack Night to build a new time system is one of the ways
in which this manifests.

We don't expect Lightning Time to be used by anyone outside our universe, but it's here for you if you want to use it. (It's REALLY COOL too!)

And if you go to Purdue or will go to Purdue soon and have stumbled across this post, we hope you'll check out Purdue Hackers :)
