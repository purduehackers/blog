...
Bolts are the "hours", zaps are the "minutes", sparks are the "seconds", and charges are the "milliseconds"; however, because the units of Lightning Time are divided up differently, it runs more slowly, so these units don't map very well to the time system we're used to. 1 bolt equals exactly 90 minutes; 1 zap equals a little over 5 and a half minutes; one spark equals about 21 seconds; one charge equals about 1.3 seconds.
