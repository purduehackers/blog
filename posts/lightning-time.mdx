...
each color with these four parts. It looks something like this:
...
The static values in this demo are the same as the values highlighted in teal in the screenshot above—they're the values
we've picked for Purdue Hackers, and the default across the Lightning Time tooling we've built. (AKA, you don't have to think
about colors when you use Lightning Time; you can just use our default colors.) But you can replace them with anything!
...
Below is also a tool to help you convert between "normal" time and Lightning Time. Pick some of your favorite times and convert
...
- Zaps and sparks don't map very well to "normal" time; one zap lasts a little over 5 minutes, and one spark lasts about 21 seconds.
...
to end up with a color (<span className="lightning-gradient">or three</span>). We considered a few different ways to represent time as a color, before eventually landing
on Lightning Time's three-color system.

Finally, we settled on the tilde `~` as a delimeter for major units and the pipe `|` to separate the three main units from
smaller units, and came up with Lightning Time as a name for our new time system.
...
![](/posts/lightning-time/color-system-whiteboard.jpeg)

![](/posts/lightning-time/original-clock.jpeg)
...
You have to come up with an idea that you find interesting, is feasible to build, and teaches you new things, but not too many
...
We want being part of Purdue Hackers to feel like being part of a magical universe where you carve out
real time in your day to build & ship creative & technical projects with friendly and interesting people.
Coming together at Hack Night to build a new time system is one of the ways in which this guiding philosophy manifested.

We don't expect Lightning Time to be used by anyone outside our universe, but we think it's really cool, and it's here for you if you want to use it.
