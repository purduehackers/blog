...
const ColorToy = ({
  ...
  return (
    <div className="px-3 py-4 rounded-lg border-4 border-black dark:border-white/30 flex flex-col gap-4 justify-center">
      ...
      <div className="flex flex-row gap-x-2 justify-center font-mono">
        {part === LightningPart.Bolt && (
          <div className="px-2 rounded bg-gray-100 dark:bg-gray-800 text-center border-2 border-red-800">
            R
          </div>
        )}
        <input
          ...
          className={`border-2 border-black bg-white dark:bg-gray-800 text-black dark:text-gray-100 placeholder:text-black/40 dark:placeholder:text-white/40 rounded w-1/5 md:w-1/3 text-center sm:text-left px-1 outline-none ${
            !valid[0] ? "bg-red-100 dark:bg-red-900/40" : ""
          } ${
            part === LightningPart.Bolt ? "border-green-800" : "border-red-800"
          }`}
        ></input>
        {part === LightningPart.Zap && (
          <div className="px-2 rounded bg-gray-100 dark:bg-gray-800 text-center border-2 border-green-800">
            G
          </div>
        )}
        <input
          ...
          className={`border-2 border-black bg-white dark:bg-gray-800 text-black dark:text-gray-100 placeholder:text-black/40 dark:placeholder:text-white/40 rounded w-1/5 md:w-1/3 text-center sm:text-left px-1 outline-none ${
            !valid[1] ? "bg-red-100 dark:bg-red-900/40" : ""
          } ${
        ...
        ></input>
        {part === LightningPart.Spark && (
          <div className="px-2 rounded bg-gray-100 dark:bg-gray-800 text-center border-2 border-blue-800">
            B
          </div>
        )}
      </div>
      <div className="flex flex-row justify-center gap-2 md:gap-0 md:justify-between">
        {midpoints.map((midpoint, i) => (
          <div key={midpoint} className="flex flex-col gap-2 justify-center">
            ...
            ></div>
            <div
              className={`text-xs rounded bg-gray-100 dark:bg-gray-800 text-center font-mono border-2 ${
                part === LightningPart.Bolt
                  ? "border-red-800"
                  : part === LightningPart.Zap
                    ? "border-green-800"
